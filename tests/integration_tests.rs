...
use bytes::Bytes;
use rusty_s4i_io::{TransportConfig, TransportId, TransportManager};
...
async fn test_url_parsing() {
    ...
    assert_eq!(
        tcp_config.transport_type,
        rusty_s4i_io::transport::TransportType::Tcp
    );

    let udp_config = TransportConfig::from_url("udp://192.168.1.1:9000").unwrap();
    assert_eq!(
        udp_config.transport_type,
        rusty_s4i_io::transport::TransportType::Udp
    );

    let http_config = TransportConfig::from_url("http://example.com").unwrap();
    assert_eq!(
        http_config.transport_type,
        rusty_s4i_io::transport::TransportType::Http
    );
}
...
async fn test_error_handling() {
    ...
    let result = manager
        .send("non_existent", &TransportId::Connection(0), Bytes::new())
        .await;
    assert!(result.is_err());
}
